...
The `pre-push` hook (`.githooks/pre-push`, wired up via `npm install` → the `prepare` script → `git config core.hooksPath .githooks`) runs `npm run gate` again automatically and **blocks the push** if it fails. That's the enforcement point — step 3 is just so you find out sooner. If the hook blocks you, fix and push again; don't reach for `--no-verify`.
...
If the user comes back with review comments or asks for changes: repeat steps 2–5 on the same branch (new commits, not amends of pushed commits, unless the user asks you to squash/rebase). The gate runs again on every push, so fixes get the same scrutiny as the original change.
